...
spec = do
    describe "Tensor Edges" $ do
        it "works if graph has no vertices" $ do
          tensorEdges (Graph.buildG (0,-1) []) (Graph.buildG (0,1) [(0,1)]) `shouldBe` Set.fromList []
          tensorEdges (Graph.buildG (0,1) [(0,1)]) (Graph.buildG (0,-1) []) `shouldBe` Set.fromList []
          tensorEdges (Graph.buildG (0,-1) []) (Graph.buildG (0,-1) []) `shouldBe` Set.fromList []
        ...
        it "works for simple graphs" $ do
          ...
          tensorEdges (Graph.buildG (0,1) [(0,1)]) (Graph.buildG (0,2) [(0,1),(1,2),(2,0)])
            `shouldBe` Set.fromList [((0,0),(1,1)),((0,1),(1,2)),((0,2),(1,0))]

    describe "Tensor Product" $ do
        it "works if graph has no vertices" $ do
          tensorGraphProduct (Graph.buildG (0,-1) []) (Graph.buildG (0,1) [(0,1)]) `shouldBe` (Graph.buildG (0,-1) [])
          tensorGraphProduct (Graph.buildG (0,1) [(0,1)]) (Graph.buildG (0,-1) []) `shouldBe` (Graph.buildG (0,-1) [])
          tensorGraphProduct (Graph.buildG (0,-1) []) (Graph.buildG (0,-1) []) `shouldBe` (Graph.buildG (0,-1) [])
        it "works if one graph is just a single vertex" $ do
        ...
        it "works for simple graphs" $ do
          ...
          tensorGraphProduct (Graph.buildG (0,1) [(0,1)]) (Graph.buildG (0,2) [(0,1),(1,2),(2,0)])
            `shouldBe` (Graph.buildG (0,5) [(0,4),(1,5),(2,3)])
